...
DiscordCall *DiscordClient::start_call_with_audio_callbacks(int64_t lobby_id, Callable received_cb, Callable captured_cb) {
	...
	auto p2 = [captured_cb](auto data, auto samples_per_channel, auto sample_rate, auto channels) {
		DiscordInt16Array *p0 = memnew(DiscordInt16Array(data, samples_per_channel * channels));
...
void DiscordClient::set_no_audio_input_threshold(real_t dbfs_threshold) {
	float p0 = (float)dbfs_threshold;
...
void DiscordClient::_bind_methods() {
	...
	ClassDB::bind_method(D_METHOD("set_no_audio_input_threshold", "dbfs_threshold"),
