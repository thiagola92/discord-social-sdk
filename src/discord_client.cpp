...
Ref<DiscordUserHandle> DiscordClient::get_current_user() {
	auto r0 = obj->GetCurrentUser();

	discordpp::UserHandle *r1_t = memnew(discordpp::UserHandle(std::move(r0)));
	Ref<DiscordUserHandle> r1 = memnew(DiscordUserHandle{ r1_t });
...
void DiscordClient::accept_activity_invite(Ref<DiscordActivityInvite> invite, Callable cb) {
...
void DiscordClient::authorize(Ref<DiscordAuthorizationArgs> args, Callable callback) {
...
void DiscordClient::get_token_from_device(Ref<DiscordDeviceAuthorizationArgs> args, Callable callback) {
...
void DiscordClient::get_token_from_device_provisional_merge(Ref<DiscordDeviceAuthorizationArgs> args, DiscordAuthenticationExternalAuthType::Enum external_auth_type, String external_auth_token, Callable callback) {
	discordpp::DeviceAuthorizationArgs p0 = *args->unwrap();
	discordpp::AuthenticationExternalAuthType p1 = (discordpp::AuthenticationExternalAuthType)external_auth_type;
	std::string p2 = std::string(external_auth_token.utf8().get_data());
	auto p3 = [callback](auto result, auto access_token, auto refresh_token, auto token_type, auto expires_in, auto scopes) {
...
void DiscordClient::send_activity_join_request_reply(Ref<DiscordActivityInvite> invite, Callable cb) {
	discordpp::ActivityInvite p0 = *invite->unwrap();
	auto p1 = [cb](auto result) {
		discordpp::ClientResult *p0_t = memnew(discordpp::ClientResult(std::move(result)));
		Ref<DiscordClientResult> p0 = memnew(DiscordClientResult{ p0_t });
...
void DiscordClient::set_noise_cancellation(bool on) {
	bool p0 = on;
...
void DiscordClient::update_rich_presence(Ref<DiscordActivity> activity, Callable cb) {
...
void DiscordClient::_bind_methods() {
	...
	ClassDB::bind_method(D_METHOD("set_noise_cancellation", "on"),
			&DiscordClient::set_noise_cancellation);
